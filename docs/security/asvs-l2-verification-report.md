...
> **ASVS Level 2, with 27 documented exceptions.** **MASVS-L1, with 1 documented exception** (plus two
> L2 controls satisfied as a bonus, not as a level claim).

Stated plainly, without a silent downgrade: 188 of 257 ASVS control rows are `satisfied` with
verified evidence, 38 are `not-applicable` with a written reason, 2 are L3-only and out of scope, and
**29 are `partial`** — each naming its own gap. Every one of those 29 is enumerated in
...
"L2 with 27 documented exceptions" is only meaningful if the exceptions are enumerable. They are.
