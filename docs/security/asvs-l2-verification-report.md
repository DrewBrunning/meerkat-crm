...
| A new authentication path (a route that mints a session) | **Partly.** The authorization matrix above enumerates every route from the live router, so a new session-minting route is at least declared and probed (its `public`/`unauth` cell). What nothing fails on is the *session-minting* property itself: the invariant that every such route carries `AuthRateLimitMiddleware` **and** `EnforceMinClientVersion` ahead of any credential work is the hand-maintained four-route group in `routes.go:50-67` (issue #692, #722), and `client_version_route_test.go` tests the middleware against that list rather than the invariant. A future auth path could ship rate-limited-but-floor-less (or neither) and stay green. Built by issue **#840**, filed by the v0.6.10 gate. |
