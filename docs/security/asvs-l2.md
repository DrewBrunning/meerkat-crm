...
| 3.3.1 | Logout/expiry invalidate session | satisfied | Logout clears cookies (`user_controller.go:262-266`); JWT expiry 96 h default (`config/config.go:192-204`); `token_version` revocation (`middleware/auth.go:141-154`); test `middleware/auth_lifecycle_test.go:59-84`, E2E `routes/session_lifecycle_test.go` |
| 3.3.2 | Re-authentication after idle | partial | Absolute 96 h expiry but **no idle timeout** (`config/config.go:192-204`). Gap vs L2's 12 h/30 min idle guidance; documented as the chosen trade-off for a personal CRM |
