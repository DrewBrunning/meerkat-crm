...
| 1.9.1 | Communications encrypted | satisfied | TLS at external reverse proxy (`docs/deployment.md:35`); HSTS emitted when HTTPS is configured (`backend/middleware/security_headers.go:43-45`, `backend/main.go:484`); SMTP STARTTLS/implicit TLS (`backend/services/mailer.go`, `SMTP_USE_TLS`) |
...
| 2.1.8 | Password strength meter | satisfied | `/check-password-strength` endpoint (`backend/controllers/user_controller.go:278-290`, route `routes/routes.go:71`); meter on `frontend/src/RegisterPage.tsx` |
...
| 3.3.1 | Logout/expiry invalidate session | satisfied | Logout clears cookies (`user_controller.go:262-266`); JWT expiry 96 h default (`config/config.go:193-203`); `token_version` revocation (`middleware/auth.go:141-154`); test `middleware/auth_lifecycle_test.go:59-84`, E2E `routes/session_lifecycle_test.go` |
| 3.3.2 | Re-authentication after idle | partial | Absolute 96 h expiry but **no idle timeout** (`config/config.go:193-203`). Gap vs L2's 12 h/30 min idle guidance; documented as the chosen trade-off for a personal CRM |
...
| 8.2.1 | Anti-caching headers for browsers | partial | SPA HTML gets `Cache-Control: no-cache` (`docker/nginx.conf:224-233`); API responses get no anti-cache headers. Gap: same as 8.1.1. |
...
| 11.1.4 | Anti-automation / anti-DoS | satisfied | Body-size limits 10 MB/1 MB (`middleware/body_limit.go:11-40`), `MaxMultipartMemory` (`main.go:460`), API rate limiter, account lockout, plus the issue #415 inventory below (search-term 256-rune cap, import row/size + session caps, bulk-500, audit-export 100k, webhook fan-out bounds, graph-depth-5) |
...
| 14.5.2 | Origin not used for authz | satisfied | Authorization derives only from the verified JWT/API token plus the `user_id` scope on every query (`backend/middleware/auth.go:66-98`, V4.1.1). `Origin` is read exclusively by the CORS policy (`backend/main.go:453-469`), which decides whether a browser may *read* a response — never whether the caller may perform the action |
| 14.5.3 | CORS strict allow-list, no "null" | satisfied | `AllowOrigins = [FrontendURL]`; `"*"` refused in release (`main.go:464-480`, `config.go:687-698`) |
