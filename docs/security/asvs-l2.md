...
| 1.9.1 | Communications encrypted | satisfied | TLS at external reverse proxy (`docs/deployment.md:32`); HSTS emitted when HTTPS is configured (`backend/middleware/security_headers.go:43-45`, `backend/main.go:484`); SMTP STARTTLS/implicit TLS (`backend/services/mailer.go`, `SMTP_USE_TLS`) |
